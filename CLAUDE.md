...
A `QTextEdit`'s **contents** are a document rather than widgets, so no QSS
rule reaches them at all: the LED page's log re-renders its own transcript on
a theme change ([§13](#13-theming)).
...
- **Style it, don't paint it.** A widget coloured through the QSS follows a
  ...
  trap: it survives the switch and keeps the old scheme's colour. The six
  places that legitimately paint are `ui/dashboard/summary_panels.py`,
  `ui/database/database_page.py`, `ui/logs/logs_page.py`,
  `ui/led/led_page.py`, `ui/widgets/led_indicator.py` and
  `ui/widgets/image_view.py`.
- **A QTextEdit's contents are a document, not widgets** — no QSS rule reaches
  them. The LED page's communication log therefore keeps its own bounded
  transcript (`_entries`, `_LOG_LIMIT`) of `(timestamp, message, kind)` and
  re-renders the lot through `theme.color()` on every switch; "Clear Log"
  clears that buffer as well as the view, or a later switch would resurrect
  the cleared lines. The same render escapes the message, because a raw
  RS232 response is arbitrary text going into an HTML span.
- **Three colours deliberately do *not* follow the theme**, because they sit on
  ...
  not against a page. The image viewer's letterbox is near-black in both
